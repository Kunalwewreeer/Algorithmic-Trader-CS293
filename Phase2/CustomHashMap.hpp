...
public:
    ...
    {
    private:
        const CustomHashMap<K, V> *hashMap;
        size_t bucketIndex;
        HashNode *currentNode;

    public:
        Iterator(const CustomHashMap<K, V> *map, size_t index, HashNode *node)
            : hashMap(map), bucketIndex(index), currentNode(node) {}
        ...
        Iterator &operator++()
        {
            ...
            {
                // Move to the next bucket with an element
                do
                {
                    bucketIndex++;
                    if (bucketIndex == hashMap->bucketCount())
                    {
                        currentNode = nullptr;
                        break;
                    }
                    currentNode = hashMap->bucketHead(bucketIndex);
                } while (!currentNode);
            }
            return *this;
        }
    ...
    // Method to get the beginning of the iterator
    Iterator begin() const
    {
        size_t index = 0;
        HashNode *node = bucketHead(index);
        while (!node && index < bucketCount() - 1)
        {
            index++;
            node = bucketHead(index);
        }
        return Iterator(this, index, node);
    }

    // Method to get the end of the iterator
    Iterator end() const
    {
